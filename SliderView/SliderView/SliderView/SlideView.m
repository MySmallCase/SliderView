...
- (void)commonInit {
    ...
    pan_ = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(panHaner:)];
...
- (void)showAt:(NSInteger)index{
    if (oldIndex_ != index) {
        ...
        UIViewController *vc = [self.dataSource SlideView:self controllerAt:index];
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(SlideView:didSwitchTo:)]) {
            [self.delegate SlideView:self didSwitchTo:index];
        }
    }
}
...
- (void)switchTo:(NSInteger)index{
    ...
    if (oldCtrl_ != nil && oldCtrl_.parentViewController == self.baseViewController) {
        ...
        UIViewController *newvc = [self.dataSource SlideView:self controllerAt:index];
...
- (void)repositionForOffsetX:(CGFloat)offsetx{
    ...
    if (panToIndex_ >= 0 && panToIndex_ < [self.dataSource numberOfControllersInSlideView:self]) {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(SlideView:switchingFrom:to:percent:)]) {
        [self.delegate SlideView:self switchingFrom:oldIndex_ to:panToIndex_ percent:fabs(offsetx)/self.bounds.size.width];
    }
}
...
- (void)backToOldWithOffset:(CGFloat)offsetx{
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (panToIndex_ >= 0 && panToIndex_ < [self.dataSource numberOfControllersInSlideView:self] && panToIndex_ != oldIndex_) {
            [oldCtrl_ beginAppearanceTransition:YES animated:NO];
            [self removeWill];
            [oldCtrl_ endAppearanceTransition];
        }
        if (self.delegate && [self.delegate respondsToSelector:@selector(SlideView:switchCanceled:)]) {
            [self.delegate SlideView:self switchCanceled:oldIndex_];
        }
    }];
}
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        ...
        if (panToIndex < 0 || panToIndex >= [self.dataSource numberOfControllersInSlideView:self]) {
            ...
            if (panToIndex != panToIndex_) {
                ...
                willCtrl_ = [self.dataSource SlideView:self controllerAt:panToIndex];
        ...
        if (panToIndex_ >= 0 && panToIndex_ < [self.dataSource numberOfControllersInSlideView:self] && panToIndex_ != oldIndex_) {
            if (fabs(offsetx) > kPanSwitchOffsetThreshold) {
                ...
                [UIView animateWithDuration:animatedTime animations:^{
                    ...
                    if (self.delegate && [self.delegate respondsToSelector:@selector(SlideView:didSwitchTo:)]) {
                        [self.delegate SlideView:self didSwitchTo:oldIndex_];
                    }
                }];
            }
            else{
