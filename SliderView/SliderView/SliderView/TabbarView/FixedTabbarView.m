...
@implementation FixedTabbarViewTabItem
@end


@implementation FixedTabbarView {
...
- (void)commonInit{
    ...
    trackView_ = [[UIImageView alloc] initWithFrame:CGRectMake(0, self.bounds.size.height - 1, self.bounds.size.width, 1)];
...
- (void)setTabbarItems:(NSArray *)tabbarItems{
    if (_tabbarItems != tabbarItems) {
        ...
        float x = 0.0f;
        NSInteger i=0;
        for (FixedTabbarViewTabItem *item in tabbarItems) {
...
- (void)switchingFrom:(NSInteger)fromIndex to:(NSInteger)toIndex percent:(float)percent{
    ...
    fromLabel.textColor = [Utility getColorOfPercent:percent between:fromItem.titleColor and:fromItem.selectedTitleColor];
    fromIamge.alpha = percent;
    fromSelectedIamge.alpha = (1-percent);
    
    if (toIndex >= 0 && toIndex < [self tabbarCount]) {
        FixedTabbarViewTabItem *toItem = [self.tabbarItems objectAtIndex:toIndex];
        ...
        toLabel.textColor = [Utility getColorOfPercent:percent between:toItem.selectedTitleColor and:toItem.titleColor];
...
- (void)setSelectedIndex:(NSInteger)selectedIndex{
    if (_selectedIndex != selectedIndex) {
        ...
        if (selectedIndex >= 0 && selectedIndex < [self tabbarCount]) {
            FixedTabbarViewTabItem *toItem = [self.tabbarItems objectAtIndex:selectedIndex];
...
- (void)tapAction:(UITapGestureRecognizer *)tap{
    ...
    if (self.delegate) {
        [self.delegate SlideTabbar:self selectAt:i];
    }
}
