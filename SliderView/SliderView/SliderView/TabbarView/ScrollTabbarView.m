...
- (void)setTabbarItems:(NSArray *)tabbarItems{
    if (_tabbarItems != tabbarItems) {
        ...
        float x = 0.0f;
        NSInteger i=0;
        for (ScrollTabbarItem *item in tabbarItems) {
...
- (void)switchingFrom:(NSInteger)fromIndex to:(NSInteger)toIndex percent:(float)percent{
    ...
    fromLabel.textColor = [Utility getColorOfPercent:percent between:self.tabItemNormalColor and:self.tabItemSelectedColor];
    
    UILabel *toLabel = nil;
    if (toIndex >= 0 && toIndex < [self tabbarCount]) {
        toLabel = (UILabel *)[scrollView_ viewWithTag:kLabelTagBase+toIndex];
        toLabel.textColor = [Utility getColorOfPercent:percent between:self.tabItemSelectedColor and:self.tabItemNormalColor];
    }
